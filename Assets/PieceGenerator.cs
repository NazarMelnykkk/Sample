...
public class PieceGenerator : MonoBehaviour
{
    ...
    [Header("References")]
    [SerializeField]
    private Transform _pickerWheelTransform;
    ...
    {
        _pieceAngle = 360f / _wheelOfFortuneSettingController.GetPieces().Length;
        ...
        float pieceWidth = Mathf.Lerp(_pieceMinSize.x, _pieceMaxSize.x, 1f - Mathf.InverseLerp(PiecesMin, PiecesMax, _wheelOfFortuneSettingController.GetPieces().Length));
        float pieceHeight = Mathf.Lerp(_pieceMinSize.y, _pieceMaxSize.y, 1f - Mathf.InverseLerp(PiecesMin, PiecesMax, _wheelOfFortuneSettingController.GetPieces().Length));
        ...
        for (int i = 0; i < _wheelOfFortuneSettingController.GetPieces().Length; i++)
        {
            DrawPiece(i);
        }
    }
    ...
    {
        WheelPiece piece = _wheelOfFortuneSettingController.GetPieces()[index];
    ...
    public WheelPiece GetSelectedPiece()
    {
        ...
        return _wheelOfFortuneSettingController.GetPieces()[selectedPieceIndex];
    }
    ...
    {
        _pickerWheelTransform.localScale = new Vector3(_wheelOfFortuneSettingController.GetSize(), _wheelOfFortuneSettingController.GetSize(), 1f);

        if (_wheelOfFortuneSettingController.GetPieces().Length > PiecesMax || _wheelOfFortuneSettingController.GetPieces().Length < PiecesMin)
        {
